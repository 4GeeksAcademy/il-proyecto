    ...
    def fill_db_with_example_data():
        ...
        try:
            ...
            podcasts = [
                ...
                Resource(resource_type_id=3, url="https://open.spotify.com/show/7jQV4EcpvsV9UcukCZ3j3A", title="The Hilarious World of Depression", description="A show about clinical depression with laughs? Comedian John Moe talks to fellow comedians who have dealt with this sad part of life.", psychologist_id=3, created_at=base_time - timedelta(days=5)),
                Resource(resource_type_id=3, url="https://open.spotify.com/show/4XPl3uEEL9hvqMkoZrzbx5", title="All In The Mind", description="An exploration of all things mental, All in the Mind is about the brain and behaviour, and the fascinating connections between them.", psychologist_id=4, created_at=base_time - timedelta(days=1)),
                Resource(resource_type_id=3, url="https://open.spotify.com/show/6SzvjB6jhHnh4o7zA5Vx2Y", title="Anxiety Slayer", description="Award-winning Anxiety Slayer podcast is a leading resource for anyone who wants to soothe anxiety and live a more peaceful life.", psychologist_id=5, created_at=datetime.now())
            ]
            db.session.add_all(podcasts)
            db.session.commit()

            videos = [
                Resource(resource_type_id=2, url="https://www.youtube.com/watch?v=IW-9eOPAWdA", title="Understanding Depression", description="Clinical Psychologist Ali Mattu provides a comprehensive overview of clinical depression, its causes, and treatments.", psychologist_id=6, created_at=datetime.now()),
                Resource(resource_type_id=2, url="https://www.youtube.com/watch?v=F2hc2FLOdhI", title="What is Anxiety?", description="This video by the World Health Organization explains what anxiety is, its symptoms, and how to seek help.", psychologist_id=7, created_at=base_time - timedelta(days=2, hours=2)),
                Resource(resource_type_id=2, url="https://www.youtube.com/watch?v=GOK1tKFFIQI", title="The Science of Well-Being", description="Yale Professor Laurie Santos discusses the psychology of happiness and provides tips on how to live a happier life.", psychologist_id=8, created_at=base_time - timedelta(days=3, hours=1)),
                Resource(resource_type_id=2, url="https://www.youtube.com/watch?v=tzNq-H-r1TI", title="How to manage stress", description="This TEDx talk explores various stress management techniques and the science behind them.", psychologist_id=9, created_at=base_time - timedelta(days=4)),
                Resource(resource_type_id=2, url="https://www.youtube.com/watch?v=nCrjevx3-Js", title="Mindfulness for Beginners", description="An introductory guide to mindfulness meditation, offering practical steps for beginners to follow.", psychologist_id=10, created_at=base_time - timedelta(days=5, hours=2))
            ]
            db.session.add_all(videos)
            db.session.commit()
            
            articles = [
                Resource(resource_type_id=1, url="https://www.psicoactiva.com/blog/como-mejorar-la-autoestima/", title="Cómo mejorar la autoestima", description="Una guía completa sobre cómo construir la confianza en uno mismo y mejorar la autoestima con estrategias prácticas y basadas en la evidencia.", psychologist_id=1, created_at=base_time - timedelta(days=1, minutes=30)),
                Resource(resource_type_id=1, url="https://www.clinicbarcelona.org/noticias/ansiedad-como-detectarla-y-afrontarla", title="Ansiedad: cómo detectarla y afrontarla", description="Este artículo ofrece varias estrategias de afrontamiento para la ansiedad, ayudando a los lectores a entender y manejar su ansiedad de manera más efectiva.", psychologist_id=2, created_at=base_time - timedelta(days=2, minutes=25)),
                Resource(resource_type_id=1, url="https://www.mentalhealth.org.uk/a-to-z/d/depression", title="Los efectos de la depresión en el cuerpo", description="Un vistazo a cómo la depresión afecta físicamente el cuerpo, explorando las conexiones entre la salud mental y física.", psychologist_id=3, created_at=base_time - timedelta(days=3, minutes=20)),
                Resource(resource_type_id=1, url="https://medlineplus.gov/spanish/ency/article/003211.htm", title="Problemas de salud relacionados con el estrés", description="Medline Plus enumera varios problemas de salud comunes asociados con el estrés y cómo resolverlos.", psychologist_id=4, created_at=base_time - timedelta(days=4, minutes=15)),
                Resource(resource_type_id=1, url="https://www.redalyc.org/pdf/1805/180521538001.pdf", title="Meditación mindfulness para la ansiedad", description="Un estudio que discute cómo la meditación mindfulness puede ayudar significativamente a reducir los niveles de ansiedad.", psychologist_id=5, created_at=base_time - timedelta(days=5, minutes=10)),
                Resource(resource_type_id=1, url="https://www.who.int/es/news-room/fact-sheets/detail/depression", title="Qué saber sobre la depresión", description="La Organización Mundial de la Salud proporciona una visión detallada de la depresión, incluyendo síntomas, tratamientos y diferentes tipos.", psychologist_id=6, created_at=base_time - timedelta(days=1, minutes=30)),
                Resource(resource_type_id=1, url="https://espanol.nami.org/About-Mental-Illness/Mental-Health-Conditions/Depression", title="¿Qué es la Depresión?", description="La Asociación Nacional de Salud Mental define la depresión, sus síntomas, riesgos y opciones de tratamiento.", psychologist_id=7, created_at=datetime.now()),
                Resource(resource_type_id=1, url="https://www.infosalus.com/salud-investigacion/noticia-claves-entender-sindrome-burnout-20190606175327.html", title="Claves para entender el síndrome de 'burnout'", description="Explora el concepto de 'burnout', sus causas, síntomas y cómo manejar este tipo de estrés laboral.", psychologist_id=8, created_at=datetime.now()),
                Resource(resource_type_id=1, url="https://www.fundacioncadah.org/web/articulo/la-meditacion-como-herramienta-para-el-tratamiento-del-tdah.html", title="Meditación como tratamiento para el TDAH", description="Discute el uso de la meditación como herramienta efectiva para manejar los síntomas del TDAH en niños y adultos.", psychologist_id=9, created_at=datetime.now()),
                Resource(resource_type_id=1, url="https://www.psicoactiva.com/blog/estrategias-para-el-manejo-del-estres/", title="Estrategias para el manejo del estrés", description="Este artículo proporciona técnicas prácticas y consejos para gestionar el estrés diario y mejorar el bienestar emocional.", psychologist_id=10, created_at=base_time - timedelta(days=1, minutes=30)),
            ]
